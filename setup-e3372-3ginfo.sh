...
# Установка/удаление мониторинга Huawei E3372 (HiLink) + панели luci-app-3ginfo-lite
# на OpenWrt 25 (пакетный менеджер apk).
#
# Запуск без аргументов показывает МЕНЮ:
#   1) Установить   — драйверы, GNU wget, репозиторий 4IceG, панель, (опц.) интерфейс
#   2) Удалить      — снести всё, что поставил пункт 1 (кроме общесистемной инфраструктуры)
#   0) Выход        — просто Enter тоже выходит, установка НЕ запускается
#
# Можно и без меню, аргументом:  install | uninstall
#   sh setup-e3372-3ginfo.sh install
#   sh setup-e3372-3ginfo.sh uninstall
#
# Меню требует запуска ФАЙЛОМ (sh setup-e3372-3ginfo.sh), а не через '| sh':
# при запуске через пайп stdin занят скриптом и read из меню не сработает.
#
# Для установки роутеру нужен доступ в интернет (через модем или другой аплинк) —
# качаются пакеты и ключ репозитория.
#
...
# ===========================================================================
#  УДАЛЕНИЕ
#  Зеркалит standalone uninstall-e3372-3ginfo.sh — логика та же.
# ===========================================================================
do_uninstall() {
...
case "$ans" in
  y|Y|yes|YES) ;;
  *) echo "Отменено. Ничего не удалено."; return 0 ;;
esac
...
menu() {
  while :; do
    printf "\n"
    printf "  Huawei E3372 (HiLink) + 3ginfo-lite — установщик  (lastik9/openwrt-huawei-e3372)\n\n"
    echo   "  1) Установить  (драйверы, GNU wget, репозиторий 4IceG, панель, интерфейс)"
    echo   "  2) Удалить     (снести всё, что поставил пункт 1)"
    echo   "  0) Выход"
    printf "  Выбор: "
    read -r _ch
    case "$_ch" in
...
case "${1:-menu}" in
  menu)
    if [ -t 0 ]; then
      ...
      echo "!! Меню требует интерактивного ввода, а скрипт запущен через пайп."
      echo "!! Скачай файл и запусти его:"
      echo "!!   wget -O setup-e3372-3ginfo.sh https://raw.githubusercontent.com/lastik9/openwrt-huawei-e3372/main/setup-e3372-3ginfo.sh"
      echo "!!   sh setup-e3372-3ginfo.sh"
      echo "!! Либо укажи действие аргументом: install | uninstall"
      exit 1
    fi
    ;;
  install)          do_install ;;
  uninstall|remove) do_uninstall ;;
  *) die "Неизвестно: '$1'. Используй: (без аргумента = меню) | install | uninstall" ;;
esac

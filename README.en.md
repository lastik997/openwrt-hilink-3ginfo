...
5. Installs `luci-app-3ginfo-lite` + the Russian locale and writes the modem address `192.168.8.1` into the panel config.
6. *(Optional, with confirmation)* finds the modem's network interface by USB VID `12d1`, creates `interface LTE` (proto dhcp), adds it to the `wan` firewall zone and binds it to the panel.
...
uci set network.LTE.device='eth2'      # use your own
uci commit network
...
Then in the panel (**3ginfo-lite → Configuration**) select Interface `LTE`; the modem address `192.168.8.1` is already set by the script.
...
ifstatus LTE | grep l3_device   # modem interface
```
